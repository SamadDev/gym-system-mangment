...
const totalItemsPrice = computed(() => {
    ...
    return state.order.items?.reduce((sum, item) => {
        return sum + (item.quantity * item.product_price || 0);
    }, 0) || 0;
});
...
watch(() => state.order.order_type, (newType) => {
    if (newType === 'shipping') {
        state.order.shipping_type = 'air'; // Default to 'air' when switching to shipping
    } else {
...
watch(() => state.order.local_delivery, (isEnabled) => {
    if (!isEnabled) {
        state.order.customer_address_id = '';
    }
});
...
const createOrder = async () => {
    ...
    const formData = createFormData({
        ...
        shipping_type: isShippingOrder.value ? state.order.shipping_type : null,
        local_delivery: state.order.local_delivery ? 1 : 0,
        customer_address_id: state.order.local_delivery ? state.order.customer_address_id : '',
    ...
    try {
        await axiosRequest.post("/admin/orders/create", formData, {notify: true});
...
const handleCustomerSelect = (customer) => {
    ...
    console.log(customerAddresses.value);
};
                    ...
                    <div class="xl:col-span-2 space-y-6">
                        ...
                        <div class="panel">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    <label for="store">{{ translate('store') }}</label>
                                    <vue-select
                                        id="store"
                                        ref="storeRef"
                                        model="name"
                                        ...
                                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                                            ...
                                            <div class="lg:col-span-2">
                                                ...
                                                <div class="relative w-full aspect-square max-w-20">
                                                    <div v-if="item.image_preview" class="relative group">
