...
const handleChangeStatus = (data) => {
    eventBus.emit('openInvoiceChangeStatusModal', data);
}
...
const handleDelete = async (data) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                let url = "/admin/invoices/" + invoice_id + "/delete";
...
<template>
    <data-table
        :columns="columns"
        :url="`/admin/invoices/data${props.customerId ? '?filters[customer_id]=' + props.customerId : ''}`"
        reloadTableEvent="reloadInvoiceDatatable"
    >
            ...
            <div class="customer-info">
                <div class="customer-details">
                    ...
                    <div v-if="data.value.customer.phone_number">
                        <i class="fas fa-phone-alt text-gray-700"></i> <span>{{
                            data.value.customer.phone_number
                        }}</span>
                    </div>
                </div>
            </div>
        ...
        <template #action="{ data }">
            <div class="flex items-center">
                <view-button v-if="can('view_order')"
                             @click="() => router.push({ name: 'view_invoice', params: { id: data.value.id } })"
                />
                <edit-button v-if="can('edit_invoice')" class="ml-1"
                             @click="() => router.push({ name: 'edit_invoice', params: { id: data.value.id } })"/>
                <delete-button v-if="can('delete_invoice')" class="mx-1" @click="handleDelete(data.value)"/>
                <more-action-button v-if="can('view_invoice')">
                    <more-action-button-item v-if="can('change_status_invoice')" icon="fas fa-sync-alt"
