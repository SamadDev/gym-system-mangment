...
const invoiceNumber = computed(() => state.invoice.invoice_number || 'N/A');
const customerName = computed(() => state.invoice.customer?.name || 'N/A');
const customerEmail = computed(() => state.invoice.customer?.email || 'N/A');
const createdBy = computed(() => state.invoice.user?.name || 'N/A');
const invoiceItems = computed(() => state.invoice.items || []);
const invoiceDate = computed(() => state.invoice.invoice_date || 'N/A');
const subtotal = computed(() => Number(state.invoice.subtotal) || 0);
const discount = computed(() => Number(state.invoice.discount) || 0);
const total = computed(() => Number(state.invoice.total) || 0);
const currencyType = computed(() => state.invoice.currency_type || {});
const isPaid = computed(() => Number(state.invoice.is_paid) || 0);

const fetchInvoiceData = async () => {
    try {
        state.loading = true;
        const res = await axiosRequest.get('/admin/invoices/' + invoiceId + '/show', {});
        state.invoice = res.data.data;
    } catch (error) {
    ...
    <div>
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex items-center gap-3">
                <edit-button-text v-if="can('edit_invoice')" :href="`/admin/invoices/${invoiceId}/edit`"/>
        ...
        <!-- Invoice Content -->
        <div v-else class="invoice-container print:space-y-4">
            <!-- Compact Header -->
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-sm text-gray-500 dark:text-gray-400">
                                    {{ moment(invoiceDate).format('MMMM DD, YYYY') }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="text-right">
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                {{ isPaid === 1 ? translate('paid') : isPaid === 2 ? translate('partially_paid') : translate('unpaid') }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 print:grid-cols-3 gap-4 mb-4 print:mb-3">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-3">
                    <div class="flex items-center gap-2 mb-2">
                        <IconPackage class="w-4 h-4 text-green-600"/>
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ translate('invoice') }}</h3>
                    </div>
                    <div class="text-sm text-gray-900 dark:text-white">{{ invoiceNumber }}</div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mb-4 print:mb-3">
                <div class="p-4">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ translate('invoice_items') }}</h3>
                    </div>

                    <div v-if="invoiceItems && invoiceItems.length > 0" class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-gray-200 dark:border-gray-700">
                                    <th class="text-left py-2 px-2">#</th>
                                    <th class="text-left py-2 px-2">{{ translate('item') }}</th>
                                ...
                                <tr v-for="(item, index) in invoiceItems" :key="item.id"
                                    ...
                                    <td class="py-2 px-2">
                                        <div class="flex items-center gap-2">
                                            <span>{{ item.invoice_item || 'N/A' }}</span>
                                            <span v-if="item.order_id" class="inline-flex items-center gap-1 px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs rounded">
                                                <IconCheckCircle class="w-3 h-3"/>
                                                {{ translate('order') }}
                                            </span>
                                        </div>
                                    </td>
                                    <td class="py-2 px-2 text-right font-medium">{{ formatPrice(item.amount, currencyType) }}</td>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                <div class="p-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-2">
                            <div v-if="state.invoice.note" class="text-sm">
                                <span class="font-medium text-gray-700 dark:text-gray-300">{{ translate('note') }}:</span>
                                <span class="text-gray-900 dark:text-white ml-2">{{ state.invoice.note }}</span>
                            </div>

                            <div v-if="state.invoice.attachment" class="text-sm">
                                <span class="font-medium text-gray-700 dark:text-gray-300">{{ translate('attachment') }}:</span>
                                <a :href="state.invoice.attachment" target="_blank"
