...
const handleDelete = async (data) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                let url = "/admin/services/" + service_id + "/delete";
                const res = await axiosRequest.delete(url, {}, {notify: true});
                eventBus.emit('reloadServiceDatatable');
            } catch (error) {
                console.log(error);
            }
        }
...
<template>
    <data-table
        ...
        reloadTableEvent="reloadServiceDatatable"
    >
        <template #type="{ data }">
        ...
        <template #action="{ data }">
            <div class="flex items-center">
                <edit-button v-if="can('edit_service')" @click="handleEdit(data.value)"/>
                <delete-button v-if="can('delete_service')" class="mx-1" @click="handleDelete(data.value)"/>
            </div>
        </template>
    </data-table>
</template>
